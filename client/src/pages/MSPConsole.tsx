...
function riskBar(val: number) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
      ...
      <span style={{ fontSize: "0.9375rem", fontWeight: 700, color, minWidth: 28, textAlign: "right" }}>{val}</span>
    </div>
  );
}
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#0a0f1e", color: "#e2e8f0" }}>
      ...
      <div style={{ paddingTop: 68, minHeight: "100vh" }}>
        ...
        <div style={{ padding: "1.25rem 2rem", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: "0.875rem", color: "#475569", textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: "0.2rem" }}>MSP Command Center</div>
          ...
          <div style={{ borderRight: selected ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
            ...
            <div style={{ padding: "1rem 2rem", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", gap: "1rem" }}>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search tenants..."
                style={{ padding: "0.5rem 1rem", backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, color: "#e2e8f0", fontSize: "0.82rem", outline: "none", flex: 1, maxWidth: 300 }} />
              <div style={{ display: "flex", gap: "0.5rem" }}>
                {["All", "Critical", "Warning"].map(f => (
                  <button key={f} style={{ padding: "0.375rem 0.875rem", borderRadius: 9999, fontSize: "0.875rem", fontWeight: 600, border: "1px solid rgba(255,255,255,0.08)", backgroundColor: "transparent", color: "#64748b", cursor: "pointer" }}>{f}</button>
                ))}
              </div>
            </div>
                ...
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    {["Tenant", "Industry", "Identities", "Risk Score", "Orphans", "AI Agents", "Incidents", "MRR", "Status", ""].map(h => (
                      <th key={h} style={{ padding: "0.625rem 1rem", textAlign: "left", fontSize: "0.8125rem", color: "#475569", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={t.id}
                        ...
                        <td style={{ padding: "0.875rem 1rem" }}>
                          ...
                          <div style={{ fontSize: "0.68rem", fontFamily: "monospace", color: "#475569" }}>{t.id}</div>
                        </td>
                        <td style={{ padding: "0.875rem 1rem", fontSize: "0.9375rem", color: "#64748b" }}>{t.industry}</td>
                        <td style={{ padding: "0.875rem 1rem", fontSize: "0.85rem", fontWeight: 600, color: "#94a3b8" }}>{t.identities.toLocaleString()}</td>
                        <td style={{ padding: "0.875rem 1rem", minWidth: 140 }}>{riskBar(t.risk)}</td>
                        <td style={{ padding: "0.875rem 1rem", fontSize: "0.85rem", color: t.orphans > 50 ? "#ef4444" : t.orphans > 20 ? "#f59e0b" : "#10b981", fontWeight: 600 }}>{t.orphans}</td>
                        <td style={{ padding: "0.875rem 1rem", fontSize: "0.85rem", color: "#a78bfa", fontWeight: 600 }}>{t.ai_agents}</td>
                        <td style={{ padding: "0.875rem 1rem" }}>
                          <span style={{ fontSize: "0.85rem", fontWeight: 700, color: t.incidents > 0 ? "#ef4444" : "#10b981" }}>{t.incidents}</span>
                        </td>
                        <td style={{ padding: "0.875rem 1rem", fontSize: "0.85rem", fontWeight: 700, color: "#10b981" }}>{t.mrr}</td>
                        <td style={{ padding: "0.875rem 1rem" }}>
                          <span style={{ padding: "0.2rem 0.625rem", borderRadius: 9999, fontSize: "0.8125rem", fontWeight: 700, backgroundColor: st.bg, color: st.color, border: `1px solid ${st.border}` }}>{st.label}</span>
                        </td>
                        <td style={{ padding: "0.875rem 1rem" }}>
          ...
          {selected && (
            <div style={{ padding: "1.5rem", overflowY: "auto", maxHeight: "calc(100vh - 200px)" }}>
              <div style={{ marginBottom: "1.25rem" }}>
                <div style={{ fontSize: "0.8125rem", color: "#475569", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "0.25rem" }}>{selected.id} · {selected.industry}</div>
                  ...
                  return (
                    <div key={k.label} style={{ backgroundColor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "1rem" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.375rem" }}>
                        <div style={{ fontSize: "0.68rem", color: "#475569", textTransform: "uppercase", letterSpacing: "0.08em" }}>{k.label}</div>
              ...
              <div style={{ marginBottom: "1.5rem" }}>
                <div style={{ fontSize: "0.8125rem", color: "#475569", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "0.75rem" }}>Quick Actions</div>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {[
                    ...
                    <button key={a.label}
                      style={{ width: "100%", textAlign: "left", padding: "0.75rem 1rem", backgroundColor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 8, color: a.color, fontSize: "0.8rem", fontWeight: 500, cursor: "pointer", transition: "all 0.12s" }}
              ...
              <div style={{ padding: "1rem", backgroundColor: "rgba(16,185,129,0.05)", border: "1px solid rgba(16,185,129,0.15)", borderRadius: 10 }}>
                <div style={{ fontSize: "0.875rem", color: "#10b981", fontWeight: 700, marginBottom: "0.375rem" }}>MRR CONTRIBUTION</div>
                <div style={{ fontSize: "1.75rem", fontWeight: 900, color: "#10b981" }}>{selected.mrr}</div>
                <div style={{ fontSize: "0.875rem", color: "#64748b" }}>/ month · Node-based pricing</div>
              </div>
            </div>
          )}
        </div>
